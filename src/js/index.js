...
const projectsDisplay = {
  ...
  "velora-ai": {
    title: "Velora AI",
    description: {
      pt: "Sistema financeiro com IA para categorização automática e análise de gastos.",

      en: "AI-powered financial system for automatic categorization and expense analysis.",

      fr: "Système financier avec IA pour catégorisation automatique et analyse des dépenses.",
  ...
  "documind-ai": {
    title: "DocuMind AI",
    description: {
      pt: "Assistente RAG com embeddings para análise de documentos e respostas inteligentes.",

      en: "RAG assistant with embeddings for document analysis and smart answers.",

      fr: "Assistant RAG avec embeddings pour analyser des documents et répondre intelligemment.",

      es: "Asistente RAG con embeddings para analizar documentos y responder inteligentemente."
    }
  },
  ...
  "botflix": {
    title: "BotFlix",
    description: {
      pt: "Assistente de IA que recomenda filmes e séries com base no perfil do usuário.",

      en: "AI assistant that recommends movies and TV shows based on user profile.",

      fr: "Assistant IA qui recommande films et séries selon le profil utilisateur.",

      es: "Asistente de IA que recomienda películas y series según el perfil del usuario."
    }
  },


  "sniperprice-ai": {
    title: "SniperPrice AI",
    description: {
      pt: "Sistema de monitoramento de preços com automações via n8n.",

      en: "Price monitoring system with n8n automations.",

      fr: "Système de suivi des prix avec automatisations via n8n.",

      es: "Sistema de monitoreo de precios con automatizaciones con n8n."
    }
  },
...
const certificateLeftArrow = document.querySelector(".certificate-arrow-left");
const certificateRightArrow = document.querySelector(".certificate-arrow-right");

if (certificatesSlider && certificateLeftArrow && certificateRightArrow) {
  ...
  certificatesSlider.addEventListener("scroll", () => {
    const maxScroll = certificatesSlider.scrollWidth - certificatesSlider.clientWidth;

    if (certificatesSlider.scrollLeft >= maxScroll - 5) {
      certificatesSlider.scrollLeft = 0;
    }

    if (certificatesSlider.scrollLeft <= 0) {
      certificatesSlider.scrollLeft = 1;
    }
  });
  ...
  let isDraggingCertificates = false;
  let certificateStartX = 0;
  let certificateScrollLeft = 0;

  certificatesSlider.addEventListener("mousedown", (event) => {
    isDraggingCertificates = true;
    certificateStartX = event.pageX - certificatesSlider.offsetLeft;
    certificateScrollLeft = certificatesSlider.scrollLeft;
  });

  certificatesSlider.addEventListener("mouseleave", () => {
    isDraggingCertificates = false;
  });
  ...
  certificatesSlider.addEventListener("mousemove", (event) => {
    if (!isDraggingCertificates) return;

    event.preventDefault();

    const currentX = event.pageX - certificatesSlider.offsetLeft;
    const distance = currentX - certificateStartX;

    certificatesSlider.scrollLeft = certificateScrollLeft - distance;
  });
}